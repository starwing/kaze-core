...
static int Lctx_isread(lua_State *L) {
    kz_Context *ctx = (kz_Context*)luaL_checkudata(L, 1, LKZ_Context);
    return kz_isread(ctx) ? 1 : 0;
}

static int Lctx_buffer(lua_State *L) {
    kz_Context *ctx = (kz_Context*)luaL_checkudata(L, 1, LKZ_Context);
    if (!kz_isread(ctx))
        return luaL_error(L, "attempt to call 'buffer' of a write context, "
                "use 'commit' instread");
    else {
        size_t dlen, len;
...
static int Lctx_commit(lua_State *L) {
    kz_Context *ctx = (kz_Context*)luaL_checkudata(L, 1, LKZ_Context);
    int r;
    if (kz_isread(ctx)) {
        r = kz_commit(ctx, 0);
...
static int Lreadcontext(lua_State *L) {
    ...
    int r = kz_read(S, ctx);
    if (r != KZ_OK && r != KZ_AGAIN)
        return lkz_pusherror(L, r);
